...
@section('content')
  <section id="page-header" style="background: url('{{ url($esilan->fullImgPathOrDefault()) }}') no-repeat 0 16%; background-size: cover;">
    <div class="bg-hover">
        <div class="container header-content clearfix">
            {{-- <a href="{{ 'upload/'.$esilan->img }}" target="_blank" data-jbox-image="gallery1"> --}}
              {{ HTML::image($esilan->fullImgPathOrDefault(), $esilan->imgName.' logo', array( 'width' => 160, 'height' => 212, 'class' => 'img-fullable' )) }}
            {{-- </a> --}}
            <article>
                <h2>{{ $esilan->name }}</h2>
